# FastAPI Mock Model Service

Главной задачей данного сервиса является построение оптимальной архитектуры Python приложения под задачи, связанные с использованием ML модели.
...
Приложение имеет единственный endpoint, который применяет модель к переданным входным данным, и отдает результат клиенту.
...
## Контракт endpoint'а вызова модели

- URI - /api/v1/model/execute
- Метод запроса - POST
- Схема тела запроса: { "query": string }
- Схема тела ответа: { "result": boolean }

## Проверка работоспособности

Для проверки работоспособности приложения и детального ознакомления с контрактом endpoint можно воспользоваться Swagger-UI, доступным по ссылке http://localhost:8000/docs при стандартных настройках.
![img.png](img.png)
или OpenApi файлом, расположенному по адресу http://localhost:8000/openapi.json, после запуска приложения.
...
2. Архитектура приложения проектировалась со ссылкой на принципы DDD и SOLID. В приложении в отличие от принятых 3 слоев, разделение на 2 слоя, т.к. главная и единственная функция приложения не имеет состояния, то и уровня для взаимодействия с доменной моделью нету. По итогу есть слой user_interface для взаимодействия с клиентами при помощи разных каналов связи и слой use_case для выполнения бизнес логики, которая не имеет состояния. Использование DDD не является краеугольным камнем, здесь могла бы быть и стандартная луковичная архитектура и просто CA, решение обосновывается легкостью масштабирования приложения, её пониманием, тестированием отдельных слоев. 
3. Для возможности быстрой замены способа валидации, слабой связи между слоями приложения и простоты тестирования используется инверсия контроля за счет инжекции зависимости ValidationUseCaseInterface. Для этого же используется именно отдельный класс, а не статический метод обработки моделью. 
...
6. В приложении выбраны и оптимально написаны логи под разные уровни важности информации. На INFO уровне получаем только необходимую информацию о результате, на уровне DEBUG детализированная информация для разработки и тестирования. Общий формат логов заменен на более информативный, чем стандартный (время, уровень записи, источник).
7. Реализована логика разделения системных логов и бизнес логов. Бизнес логи сохраняются. Бизнес логи логируются настроенным логгером "audit", чьи логи можно наблюдать и использовать в Kibana. Для этого в проект добавлен и настроен стек ELB (Elasticsearch, Logstash, Kibana). Гайд по просмотру приведен к конце описания проекта.
8. Приложение задокументировано с помощью OpenApi для прозрачности контрактов взаимодействия.
9. Приложение собирается в docker image и уборачивается docker-compose для простоты развертывания и возможности автоматизации.
10. В проект добавлены и настроены инструменты упрощающие поддержку приложения:
- Flake8 (линтер)
- Black (форматер)
- Mypy (статический типизатор)
- Pytest (юнит-тесты и интеграционные)
11. Написаны интеграционные тесты на валидацию входных данных и юнит-тесты на результат применения модели к данным.
12. Для моделей DTO использовался Pydantic. Для обеспечения гарантии переданных типов и удобства работы.
13. Поскольку одновременно с моделью может работать только один клиент, то перед вызовов модели проверяется блокировка.
...
## Подготовка Kibana к использованию

Для использования Kibana необходимо следовать следующим действиям:

1. Выполнить docker-compose up
2. Дождаться окончания инициализации сервисов.
3. Перейти в Kibana http://localhost:5601/app/management/kibana/indexPatterns
4. Выбрать "Create index pattern"
5. Заполнить "Index pattern name" значением "Index pattern name"
6. Выбрать @timestamp
7. Перейти на http://localhost:5601/app/discover
8. Выбрать в качестве индекса "model-service"

В результате должно получиться следующее:

![img_1.png](img_1.png)

P.S. Если ничего нету, попробовать выставить предыдущий день для интервала поиска.

## Возможности улучшения

- Создание pipeline для автоматизации CI/CD.
- Создание infrastructure layer при появлении доменной модели.
- Добавление alembic для создания и контроля над миграциями БД в случае, если появится работа с БД.
